...
function loadInitialData() {
    ...
    fetch('https://backend-news-html.vercel.app/get_initial_data')
...
function loadMoreData() {
    ...
    const url = `https://backend-news-html.vercel.app/get_more_data?skip=${currentCardCount}&limit=15`;
...
async function handleSearch(event) {
    ...
    if (searchInput.trim() !== "") {
        try {
            ...
            const response = await fetch(`https://backend-news-html.vercel.app/search?query=${encodeURIComponent(searchInput)}`);
            if (!response.ok) {
                throw new Error('Network response was not ok');
            }
